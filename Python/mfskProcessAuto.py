...
def mfskProcessAuto(infile, baudrate, filter_range, dev):
...
#    num_samples_analog = len(input_signal)
    ...
    input_signal = np.array(mdata['analog_channel_0'].reshape(1,-1)[0])
    ...
    SNR_threshold = {
        'lcd': [0.6,0.30,0.4], # lab: [0.6,0.20,0.4], #field,lab-wdm [0.6,0.35,0.4]
        'dmd': [0.6,0.35,0.4] # lab:[0.58,0.5,0.4], #field-wdm [0.65,0.35,0.4], [0.5,0.5,0.4] before
    }
    ...
    while i < (num_samples_analog-ceil(nsymbol)*5):
        ...
        # Code for signal strength calculation
        # with open(infile[:-4]+'_signalPSD.csv','a') as f:
        #     f.write(f'Current symbol: {current_bit:02b},')
        #     f.write(','.join(str(i) for i in calc_freq_i))
        #     f.write('\n')
        ...
        if receiving_msg == False:
            ...
            if current_bit == 0 and i - realign_i > realign_dist:
                ...
                if (np.random.rand() > 1.8) and (i + floor(srate / baudrate * 8) < num_samples_analog ):
    ...
    #print(msg_array_txt)
    savemat(infile + '_processed.mat', {'msg_array_txt':msg_array_txt[:-1], 'msg_array_save':msg_array_save[:-1]})
